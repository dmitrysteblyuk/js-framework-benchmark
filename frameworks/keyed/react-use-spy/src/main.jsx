...
const RowList = () => {
  const data = useSpy(dataState);
  const selected = useSpy(selectedState);
...
const Main = () => (
  <div className="container">
    <div className="jumbotron">
      <div className="row">
        <div className="col-md-6"><h1>React Use Spy</h1></div>
        <div className="col-md-6"><div className="row">
          ...
          <Button id="add" title="Append 1,000 rows" cb={() => dataState.$ = dataState.$.concat(buildData(1000))} />
          <Button id="update" title="Update every 10th row" cb={() => dataState.$ = update(dataState.$)} />
          <Button id="clear" title="Clear" cb={() => dataState.$ = ([])} />
          <Button id="swaprows" title="Swap Rows" cb={() => dataState.$ = swapRows(dataState.$)} />
        </div></div>
      </div>
    </div>
